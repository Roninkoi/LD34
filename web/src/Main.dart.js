...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "Blob|CanvasRenderingContext2D|DOMError|File|FileError|MediaError|MediaKeyError|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGAnimatedString|SVGRect|Screen|WebGLBuffer|WebGLProgram|WebGLShader|WebGLTexture|WebGLUniformLocation"
  },
  JSBool: {
    "^": "Interceptor;",
  ...
  ListMixin: {
    ...
    setRange$4: ["super$ListMixin$setRange$4", function(receiver, start, end, iterable, skipCount) {
      ...
      if (skipCount + $length > J.get$length$asx(iterable))
        throw H.wrapException(H.IterableElementError_tooFew());
      if (skipCount < start)
        for (i = $length - 1, t1 = iterable.length, t2 = receiver.length; i >= 0; --i) {
          t3 = start + i;
          t4 = skipCount + i;
          if (t4 >= t1)
          ...
          t4 = iterable[t4];
          if (t3 >= t2)
            return H.ioore(receiver, t3);
          receiver[t3] = t4;
        }
      else
        for (t1 = iterable.length, t2 = receiver.length, i = 0; i < $length; ++i) {
          t3 = start + i;
          t4 = skipCount + i;
          if (t4 >= t1)
            return H.ioore(iterable, t4);
